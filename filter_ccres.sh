...
#  Filter for proximal elements that are CTCF-bound using V2.hg38-cCREs-Final.bed
echo "Filtering for proximal (PLS or pELS) and CTFC-bound elements..."
awk '$6 ~ /(CTCF-bound|CTCF-only)/ && $6 ~ /(PLS|pELS)/' V2.hg38-cCREs-Final.bed > ctcf_proximal.bed

#  Match allcCRE.txt to filtered cCREs
echo "Matching cCRE IDs..."
awk -v OFS='\t' 'FNR==NR {ids[$1]; next} ($5 in ids)' ccre_ids.txt ctcf_proximal.bed > filtered_cCREs.bed
echo "Common elements saved to filtered_cCREs.bed"

#  Cleanup intermediate files
rm ccre_ids.txt ctcf_proximal.bed
